        ...
        NavigationView {
            if(self.restrictionModel.inRestrictionMode) {
                VStack(alignment: .leading) {
                    Spacer()
                    VStack {
                        Text("You're Currently in Restriction Mode").font(.title).multilineTextAlignment(.leading).bold()
                        HStack {
    ...
    var body: some View {
        VStack(alignment: .leading) {
            VStack(alignment: .leading) {
                ...
                Text(MyModel.shared.selectionToDiscourage.applicationTokens.count > 0 ? "\(MyModel.shared.selectionToDiscourage.applicationTokens.count) Selected Apps: " : "No Apps Selected").font(.title).bold().padding()
                if(MyModel.shared.selectionToDiscourage.applicationTokens.count > 0 ){
                    ScrollView {
                ...
                }.padding().familyActivityPicker(isPresented: $isDiscouragedPresented, selection: $model.selectionToDiscourage)
                //Divider()
                Label("Selected Duration: \(restrictionDuration)", systemImage: "clock")
                    ...
                    .frame(maxWidth: .infinity, alignment: .leading)
                    .cornerRadius(3)
                    .backgroundStyle(.thinMaterial).padding()
